...
At a high level, I want to get all the s3 buckets in a specific account and for each of these buckets apply a lifecycle configuration rule if none exist. Therefore, my first step is to get a list of all the buckets. To accomplish this, first I create an s3 client as shown below:
...
Now that we have a list of all buckets, we can iterate through the list and take an action, in our example we will check if this bucket has a lifecycle policy and if not then add one. We do this in two steps. 1) Check if to see if policy exist and 2) Apply a policy. The logic for each of these steps are defined in two Python functions we will discuss later.

To iterate through the list will use a simple [for loop](https://docs.python.org/3/tutorial/controlflow.html#for-statements) and access the "Name" key of Bucket object. We then use this bucket name as the argument to call the get_incomplete_mpu_policy() function to trigger the workflow.
...
Setting ```DRY_RUN``` as ```True``` means we will perform all the checks, but will not apply any changes to the bucket. This is a good practice to test your script without risking impacting your buckets.
